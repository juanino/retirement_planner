...
class RetirementPlanner:
    ...
    def calculate_accumulation_phase(self):
        ...
        account_data = {name: [] for name in accounts.keys()}
    ...
    def generate_full_projection(self):
        ...
        self.console.print(f"\n[bold green]✓[/bold green] [green]Projected total at retirement (age {self.config['retirement_age']}):[/green] [bold yellow]${retirement_balance:,.2f}[/bold yellow]")
        ...
        withdrawal_order = self.config.get('withdrawal_order', list(accounts.keys()))
        self.console.print(f"[cyan]Withdrawal order:[/cyan] {' → '.join([a.replace('_', ' ').title() for a in withdrawal_order])}")
        ...
        self.console.print(f"\n[bold green]✓[/bold green] [green]Final balance at age {final_age}:[/green] [bold yellow]${final_balance:,.2f}[/bold yellow]")
        ...
        if final_balance > 0 and final_age >= self.config['life_expectancy']:
            ...
            success_text.append(f"You are projected to have ", style="white")
            success_text.append(f"${final_balance:,.2f}", style="bold yellow")
            success_text.append(f" remaining at age {final_age}\n\n", style="white")
            success_text.append("Your retirement savings should last through your expected lifetime.", style="dim green")
            ...
            warning_text.append(f"This is {self.config['life_expectancy'] - final_age:.0f} years before expected life expectancy.\n\n", 
                              style="yellow")
        ...
        total_start = sum(acc['balance'] for acc in accounts.values())
        total_contributions = sum(acc['annual_contribution'] for acc in accounts.values())
...
def main():
    ...
    try:
        ...
        results = planner.generate_full_projection()
